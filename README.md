...
- signaling 側: WHIP から DO シグナリング + P2P への転換を実装中 ([ippoan/alc-app#129](https://github.com/ippoan/alc-app/issues/129))。`CONFIG_RELAY_SIGNALING_HOST` は既定で `alc-signaling.ippoan.org` (全デバイス共通)、site_id は device credential の cam-relay-token mint 応答から実行時に決まる
